...
Real SSS_SLAB::Get_local_energy_real_space(Array<Real,3> Ar)
{	
	return Array_sqr(Ar)/ (Real(Nx)*Real(Ny)*Real(Nz));
}


Real SSS_SLAB::Get_local_energy(Array<Complex,3> A)
{
	Array<Real,3> Ar=Array<Real,3>(reinterpret_cast<Real*>(A.data()), shape_complex_array*shape(1,1,2), neverDeleteData);
	
	if (Ny > 1) {
		Real  total = 4*Array_sqr(Ar);
...
Real SSS_SLAB::Get_local_energy_real_space(Array<Real,3> Ar, Array<Real,3> Br)
{
	return mydot(Ar, Br)/ (Real(Nx)*Real(Ny)*Real(Nz));
}

Real SSS_SLAB::Get_local_energy(Array<Complex,3> A, Array<Complex,3> B)
{
...
void SSS_SLAB::Compute_local_helicity
(
...
void SSS_SLAB::Compute_total_helicity
( 
	Array<Complex,3> Ax, Array<Complex,3> Ay, Array<Complex,3> Az, 
...
void SSS_SLAB::Compute_local_shell_spectrum_helicity
(
	Array<Complex,3> Ax, Array<Complex,3> Ay, Array<Complex,3> Az, 
...
void SSS_SLAB::Compute_shell_spectrum_helicity
(
	Array<Complex,3> Ax, Array<Complex,3> Ay, Array<Complex,3> Az, 
...
void SSS_SLAB::Compute_local_ring_spectrum_helicity
(
	Array<Complex,3> Ax, Array<Complex,3> Ay, Array<Complex,3> Az, 
...
void SSS_SLAB::Compute_ring_spectrum_helicity
(
	Array<Complex,3> Ax, Array<Complex,3> Ay, Array<Complex,3> Az, 
...
void SSS_SLAB::Compute_local_cylindrical_ring_spectrum_helicity
(
	Array<Complex,3> Ax, Array<Complex,3> Ay, Array<Complex,3> Az, 
...
//
//
void SSS_SLAB::Compute_cylindrical_ring_spectrum_helicity
(
	Array<Complex,3> Ax, Array<Complex,3> Ay, Array<Complex,3> Az, 
...
Real SSS_SLAB::Get_local_Sn(Array<Complex,3> A, Real n)
{
    Array<Real,3> Ar=Array<Real,3>(reinterpret_cast<Real*>(A.data()), shape_complex_array*shape(1,1,2), neverDeleteData);
	...
	if ((my_id == master_id) && (n==0))
		Sn -= Multiplicity_factor(0, 0, 0) * pow2(abs(Ar(0,0,0)));
...
Real SSS_SLAB::Get_local_Sn(Array<Complex,3> A, Array<Complex,3> B, Real n)
{
    Array<Real,3> Ar=Array<Real,3>(reinterpret_cast<Real*>(A.data()), shape_complex_array*shape(1,1,2), neverDeleteData);
    Array<Real,3> Br=Array<Real,3>(reinterpret_cast<Real*>(B.data()), shape_complex_array*shape(1,1,2), neverDeleteData);
	...
	// Subtract the contribution from the origin only for n=0.
	if ((my_id == master_id) && (n==0))
		Sn -= Multiplicity_factor(0, 0, 0) * Ar(0,0,0)*Br(0,0,0);
...
void SSS_SLAB::Compute_local_shell_spectrum
(
 Array<Complex,3> A,
 int n,
...
Real SSS_SLAB::Get_local_entropy(Array<Complex,3> Ax, Array<Complex,3> Ay, Array<Complex,3> Az)
{
...
Real SSS_SLAB::Get_local_entropy_scalar(Array<Complex,3> A)
{
	Array<Real,3> Ar=Array<Real,3>(reinterpret_cast<Real*>(A.data()), shape_complex_array*shape(1,1,2), neverDeleteData);
...
void SSS_SLAB::Compute_local_ring_spectrum
(
 Array<Complex,3> Ax, Array<Complex,3> Ay, Array<Complex,3> Az,
 int n,
...
void SSS_SLAB::Compute_local_cylindrical_ring_spectrum
(
 Array<Complex,3> Ax, Array<Complex,3> Ay, Array<Complex,3> Az,
 int n,
...
void SSS_SLAB::Compute_local_imag_shell_spectrum_B0
(
...
void SSS_SLAB::Compute_local_imag_ring_spectrum_B0
(
 TinyVector<Real,3> B0,
 Array<Complex,3> Ax, Array<Complex,3> Ay, Array<Complex,3> Az,
 Array<Complex,3> Bx, Array<Complex,3> By, Array<Complex,3> Bz,
...
// Not for 2D
void SSS_SLAB::Compute_local_imag_cylindrical_ring_spectrum_B0
(
 TinyVector<Real,3> B0,
 Array<Complex,3> Ax, Array<Complex,3> Ay, Array<Complex,3> Az,
 Array<Complex,3> Bx, Array<Complex,3> By, Array<Complex,3> Bz,

...
DP FFFW_SLAB::Get_local_energy_XZ_plane(Array<complx,3> A, int ny)
{
	return 0;
}

DP FFFW_SLAB::Get_local_energy_real_space(Array<complx,3> Ar)
{
...
DP FFFW_SLAB::Get_total_energy_real_space(Array<complx,3> Ar)
{
...
DP FFFW_SLAB::Get_local_energy(Array<complx,3> A)
{
/*	DP total = 0.0;
	...
	if (Ny > 1) {
		...
		total +=  Array_sqr(A(Range::all(), Ny/2, Range::all()));			
			// for ky = -Ny/2 modes that are not stored.
		
		// for kx = -Nx/2 modes that are not stored.
		if (numprocs == 1)
			total += Array_sqr(A(Nx/2, Range::all(), Range::all()));		
		...
		total -= Array_sqr(A(Range::all(), Ny/2, 0))/2;				// ky = -Ny/2
		
		// kx = -Nx/2
		if (numprocs == 1)
			total -=  Array_sqr(A(Nx/2, Range::all(), 0))/2;					
		...
		// for kx = -Nx/2 modes that are not stored.
		if (numprocs == 1)
			total += Array_sqr(A(Nx/2, 0, Range::all()));		
...
DP FFFW_SLAB::Get_total_energy(Array<complx,3> A)
{
...
DP FFFW_SLAB::Get_local_energy_XZ_plane(Array<complx,3> A, Array<complx,3> B, int ny)
{
	return 0.0;
}

DP FFFW_SLAB::Get_local_energy(Array<complx,3> A, Array<complx,3> B)
{
/*	
	...
	if (Ny > 1) {
		...
		total +=  mydot(A(Range::all(), Ny/2, Range::all()), B(Range::all(), Ny/2, Range::all()));		
			// for ky = -Ny/2 modes that are not stored.
		
			// for kx = -Nx/2 modes that are not stored.
		if (numprocs == 1)
			total += mydot(A(Nx/2, Range::all(), Range::all()), B(Nx/2, Range::all(), Range::all()));	
		...
		total -= mydot(A(Range::all(), Ny/2, 0), B(Range::all(), Ny/2, 0))/2; // ky = -Ny/2
		
		// kx = -Nx/2
		if (numprocs == 1)
			total -=  mydot(A(Nx/2, Range::all(), 0), B(Nx/2, Range::all(), 0))/2;	
		...
		total = mydot(A, B);
			...
			// for kx = -Nx/2 modes that are not stored.
		if (numprocs == 1)
			total += mydot(A(Nx/2, 0, Range::all()), B(Nx/2, 0, Range::all()));	
		...
		total -= mydot(A(Range::all(), 0, 0), B(Range::all(), 0, 0))/2;
			...
			// point(-Nx/2,0)
		if (numprocs == 1)
			total -=  real_cprod(A(Nx/2, 0, 0), B(Nx/2, 0, 0))/2;				
		
		else if (my_id == numprocs/2)
			total -=  real_cprod(A(0, 0, 0), B(0, 0, 0))/2;				
	} 
...
DP FFFW_SLAB::Get_total_energy(Array<complx,3> A, Array<complx,3> B)
{
...
DP FFFW_SLAB::Get_local_Sn(Array<complx,3> A, DP n)
{
	DP Sn = 0.0;
...
DP FFFW_SLAB::Get_total_Sn(Array<complx,3> A, DP n)
{
	DP local_total;
...
//**********************************************************************************************
//  Computes \f$ \sum  K^n |A(k)|^2/2 \f$;   k=0 excluded.
DP FFFW_SLAB::Get_local_Sn(Array<complx,3> A, Array<complx,3> B, DP n)
{
	DP Sn = 0.0;
	DP Kmag;	// Kmag = sqrt(Kx^2+Ky^2+Kz^2)
...
DP FFFW_SLAB::Get_total_Sn(Array<complx,3> A, Array<complx,3> B, DP n)
{
	DP local_total;
...
void FFFW_SLAB::Compute_local_shell_spectrum
(  
	Array<complx,3> A, 
	int n, 
	Array<DP,1> local_Sk, 
...
void FFFW_SLAB::Compute_shell_spectrum
(  
    Array<complx,3> A, 
    int n, 
    Array<DP,1> Sk
)
{
	...
	MPI_Reduce(reinterpret_cast<DP*>(local_Sk.data()), reinterpret_cast<DP*>(Sk.data()), data_size, MPI_DP, MPI_SUM, master_id, MPI_COMMUNICATOR); 
					
	MPI_Reduce(reinterpret_cast<DP*>(local_Sk_count.data()), reinterpret_cast<DP*>(Sk_count.data()), data_size, MPI_DP, MPI_SUM, master_id, MPI_COMMUNICATOR); 
...
)
{
	...
	MPI_Reduce(reinterpret_cast<DP*>(local_Sk.data()), 
						...
						data_size, MPI_DP, MPI_SUM, master_id, MPI_COMMUNICATOR); 
...
)
{
	...
	MPI_Reduce(&local_helicity1, &total_helicity1, 1, MPI_DP, MPI_SUM, master_id, MPI_COMMUNICATOR);
								
	MPI_Reduce(&local_helicity2, &total_helicity2, 1, MPI_DP, MPI_SUM, master_id, MPI_COMMUNICATOR);
								
	MPI_Reduce(&local_k2H1, &total_k2H1, 1, MPI_DP, MPI_SUM, master_id, MPI_COMMUNICATOR);
									
	MPI_Reduce(&local_k2H2, &total_k2H2, 1, MPI_DP, MPI_SUM, master_id, MPI_COMMUNICATOR);
}
...
)	
{
	...
	MPI_Reduce(reinterpret_cast<DP*>(local_H1k1.data()), reinterpret_cast<DP*>(H1k1.data()), data_size, MPI_DP, MPI_SUM, master_id, MPI_COMMUNICATOR);
					
	MPI_Reduce(reinterpret_cast<DP*>(local_H1k2.data()), reinterpret_cast<DP*>(H1k2.data()), data_size, MPI_DP, MPI_SUM, master_id, MPI_COMMUNICATOR);
	
	MPI_Reduce(reinterpret_cast<DP*>(local_H1k3.data()), reinterpret_cast<DP*>(H1k3.data()), data_size, MPI_DP, MPI_SUM, master_id, MPI_COMMUNICATOR);								  								  
					
	MPI_Reduce(reinterpret_cast<DP*>(local_H1k_count.data()), reinterpret_cast<DP*>(H1k1_count.data()), data_size, MPI_DP, MPI_SUM, master_id, MPI_COMMUNICATOR); 
...
DP FFFW_SLAB::Get_local_entropy(Array<complx,3> Ax, Array<complx,3> Ay, Array<complx,3> Az)
{
	...
	MPI_Bcast( &total_energy, 1, MPI_DP, master_id, MPI_COMMUNICATOR); 
...
DP FFFW_SLAB::Get_entropy(Array<complx,3> Ax, Array<complx,3> Ay, Array<complx,3> Az)
{

	DP local_entropy = Get_local_entropy(Ax, Ay, Az);
	
	DP entropy;
	MPI_Reduce(&local_entropy, &entropy, 1, MPI_DP, MPI_SUM, master_id, MPI_COMMUNICATOR);	
...
DP FFFW_SLAB::Get_local_entropy_scalar(Array<complx,3> A)
{
...
DP FFFW_SLAB::Get_entropy_scalar(Array<complx,3> A)
{
...
void FFFW_SLAB::Compute_local_ring_spectrum
(
	Array<complx,3> Ax, Array<complx,3> Ay, Array<complx,3> Az, 
	int n, 
...
void FFFW_SLAB::Compute_ring_spectrum
(
	Array<complx,3> Ax, Array<complx,3> Ay, Array<complx,3> Az, 
	int n, 
...
)
{
	...
	MPI_Reduce(reinterpret_cast<DP*>(local_S1k.data()), reinterpret_cast<DP*>(S1k.data()), data_size, MPI_DP, MPI_SUM, master_id, MPI_COMMUNICATOR);
						
	MPI_Reduce(reinterpret_cast<DP*>(local_S2k.data()), reinterpret_cast<DP*>(S2k.data()), data_size, MPI_DP, MPI_SUM, master_id, MPI_COMMUNICATOR);					 
...
)	
{
	...
	MPI_Reduce(reinterpret_cast<DP*>(local_H1k.data()), reinterpret_cast<DP*>(H1k.data()), data_size, MPI_DP, MPI_SUM, master_id, MPI_COMMUNICATOR); 
...
void FFFW_SLAB::Compute_local_cylindrical_ring_spectrum
( 
	Array<complx,3> Ax, Array<complx,3> Ay, Array<complx,3> Az, 
	int n, 
	Array<DP,2> local_S1k, Array<DP,2> local_S2k
...
//
//
void FFFW_SLAB::Compute_cylindrical_ring_spectrum
( 
	Array<complx,3> Ax, Array<complx,3> Ay, Array<complx,3> Az, 
	int n, 
	Array<DP,2> S1k, Array<DP,2> S2k
...
void FFFW_SLAB::Compute_local_imag_shell_spectrum_B0
( 
	TinyVector<DP,3> B0,
	Array<complx,3> Ax, Array<complx,3> Ay, Array<complx,3> Az,
	Array<complx,3> Bx, Array<complx,3> By, Array<complx,3> Bz, 
...
//
//
void FFFW_SLAB::Compute_imag_shell_spectrum_B0
(
	TinyVector<DP,3> B0,
	Array<complx,3> Ax, Array<complx,3> Ay, Array<complx,3> Az,
	Array<complx,3> Bx, Array<complx,3> By, Array<complx,3> Bz, 
...
void FFFW_SLAB::Compute_local_imag_ring_spectrum_B0
(
	TinyVector<DP,3> B0,
	Array<complx,3> Ax, Array<complx,3> Ay, Array<complx,3> Az,
	Array<complx,3> Bx, Array<complx,3> By, Array<complx,3> Bz, 
...
//
//
void FFFW_SLAB::Compute_imag_ring_spectrum_B0
(
	TinyVector<DP,3> B0,
	Array<complx,3> Ax, Array<complx,3> Ay, Array<complx,3> Az,
	Array<complx,3> Bx, Array<complx,3> By, Array<complx,3> Bz, 
...
// Not for 2D
void FFFW_SLAB::Compute_local_imag_cylindrical_ring_spectrum_B0
(
	TinyVector<DP,3> B0,
	Array<complx,3> Ax, Array<complx,3> Ay, Array<complx,3> Az,
	Array<complx,3> Bx, Array<complx,3> By, Array<complx,3> Bz, 
...
//
//
void FFFW_SLAB::Compute_imag_cylindrical_ring_spectrum_B0
(
	TinyVector<DP,3> B0,
	Array<complx,3> Ax, Array<complx,3> Ay, Array<complx,3> Az,
	Array<complx,3> Bx, Array<complx,3> By, Array<complx,3> Bz,  

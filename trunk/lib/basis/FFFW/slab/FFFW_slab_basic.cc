...
int FFFW_SLAB::Get_number_modes_in_shell(DP inner_radius, DP outer_radius)
{
...
void FFFW_SLAB::Print_large_Fourier_elements(Array<complx,3> A)
{
...
void FFFW_SLAB::Array_mult_ksqr(Array<complx,3> A)
{
	firstIndex ly;
	secondIndex lz;
...
void FFFW_SLAB::Array_divide_ksqr(Array<complx,3> A)
{
	firstIndex ly;
	secondIndex lz;
...
void FFFW_SLAB::Array_mult_V0_khat_sqr(Array<complx,3> A, TinyVector<DP,3> V0)
{
	firstIndex ly;
	secondIndex lz;
...
void FFFW_SLAB::Satisfy_strong_reality_condition_in_Array(Array<complx,3> A)
{
	...
	int array_index_minux_kx, array_index_minux_ky;
	...
	for (int lx=0; lx<local_Nx; lx++) {
		if (Get_kx(lx) != Nx/2) {  // Do not apply for kx=Nx/2
			array_index_minux_kx =  Get_ix(-Get_kx(lx));  // minuskx = Get_kx(lx);
			
			for (int ly=Ny/2+1; ly<Ny; ly++) {
				array_index_minux_ky = -Get_ky(ly);        // minusky = Get_ky(ly)
				A(lx, ly, 0) = conj(global.temp_array.plane_xy(array_index_minux_kx,array_index_minux_ky));
			}
			// for (ky=0,kz=0) line, change only the A(-kx,0,0)
			if (Get_kx(lx) < 0)
				A(lx,0,0) = conj(global.temp_array.plane_xy(array_index_minux_kx,0));
		}
	}
        
	// for kz=Nz/2; Don't do it for A(:,:,1) 2D arrays.
    if (Nz > 1)
	...
	for (int lx=0; lx<local_Nx; lx++) {
		if (Get_kx(lx) != Nx/2) {
			...
			for (int ly=Ny/2+1; ly<Ny; ly++) {
				...
				if (abs(A(lx, ly, 0)-conj(global.temp_array.plane_xy(array_index_minux_kx,array_index_minux_ky))) > MYEPS2)
					cout << "Reality condition voilated for (kx,ky,kz)=(" << Get_kx(lx) <<  "," << ly << "," << 0 << ")" << endl; 
			}
			// for (ky=0,kz=0) line
			if (Get_kx(lx) < 0) {
				if (abs(A(lx,0,0)-conj(global.temp_array.plane_xy(array_index_minux_kx,0))) > MYEPS2)
				   cout << "Reality condition voilated for (kx,ky,kz)=(" << Get_kx(lx) <<  "," << 0 << "," << 0 << ")" << endl;
			}
		}
	}
...
void FFFW_SLAB::Fill_Vz(Array<complx,3> Ax, Array<complx,3> Ay, Array<complx,3> Az)
{
...
void FFFW_SLAB::Zero_modes(Array<complx,3> Ax, Array<complx,3> Ay, Array<complx,3> Az)
{
    // Do nothing
}

void FFFW_SLAB::Zero_modes(Array<complx,3> F)
{
    // Do nothing
}
...
void FFFW_SLAB::Copy_array_complex_to_real(Array<complx,3> A, Array<DP,3> Acopy)
{
...
void FFFW_SLAB::Copy_array_real_to_complex(Array<DP,3> Acopy, Array<complx,3> A)
{
     // Dummy function: Do nothing... Note that Acopy is not allocated.
}
...
void FFFW_SLAB::Copy_realarray_complex_to_real(Array<complx,3> Ar, Array<DP,3> Ar_copy)
{
...
void FFFW_SLAB::Copy_realarray_real_to_complex(Array<DP,3> Ar_copy, Array<complx,3> Ar)
{
     // Dummy function: Do nothing... Note that Ar_copy is not allocated.
}
...
// Same for Ny
void FFFW_SLAB::Adjust_array_after_reading_reduced(Array<complx,3> A)
{

...
#include "sf_slab_alltoall_2d.h"


	
//*********************************************************************************************
SF_slab_Alltoall_2D::SF_slab_Alltoall_2D(int my_id, int numprocs, int num_iter, int Nx, int Nz): SpectralPlan_Slab_2D("SF", my_id, numprocs, Nx, Nz)
{
...
DP SF_slab_Alltoall_2D::f(string sincostr_option, int rx, int rz)
{
	DP k0 = 1;
	DP x,z;
...
void SF_slab_Alltoall_2D::Init_array()
{
	Xr_2d=0;
...
void SF_slab_Alltoall_2D::Normalize(Array<complx,2> A)
{
	A /= (2.0 * DP(Nx) * DP(Nz));
}
...
void SF_slab_Alltoall_2D::Forward_transform(string sincostr_option, Array<DP,2> Ar, Array<complx,2> A)
{
...
void SF_slab_Alltoall_2D::Inverse_transform(string sincostr_option, Array<complx,2> A, Array<DP,2> Ar)
{
	IFT_c2r_z(A);
...
void SF_slab_Alltoall_2D::Transpose(Array<DP,2> Ar, Array<complx,2> A)
{
	Alltoall(Ar,A,X);
}

void SF_slab_Alltoall_2D::Transpose(Array<complx,2> A, Array<DP,2> Ar)
{
	Alltoall(A,Ar,Z);
}

//Transforms
void SF_slab_Alltoall_2D::SinCostr_x(char sincostr_option, Array<DP,2> Ar)
{
...
void SF_slab_Alltoall_2D::ISinCostr_x(char sincostr_option, Array<DP,2> Ar)
{
    if (sincostr_option == 'S') {
		ArrayShiftLeft(Ar, 'X');
...
void SF_slab_Alltoall_2D::FT_r2c_z(Array<complx,2> A)
{
	FFTW_EXECUTE_DFT_R2C(plan_ft_r2c_z, reinterpret_cast<DP*>(A.data()), reinterpret_cast<FFTW_COMPLEX*>(A.data()));
}


void SF_slab_Alltoall_2D::IFT_c2r_z(Array<complx,2> A)
{

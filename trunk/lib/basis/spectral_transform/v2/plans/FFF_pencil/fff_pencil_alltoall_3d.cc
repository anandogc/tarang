...
//*********************************************************************************************
FFF_slab_Alltoall_3D::FFF_slab_Alltoall_3D(int my_id, int numprocs, int num_iter, int Nx, int Ny, int Nz): SpectralPlan_Slab_3D(my_id, numprocs, Nx, Ny, Nz)
{

	X_3d.resize(Nx, local_Ny, local_Nz);
	Xr_3d.resize(local_Nx, local_Ny, local_Nz*numprocs);
	...
	plan_ft_c2c_x = FFTW_PLAN_MANY_DFT(1, Nx_plan, local_Ny*local_Nz,
	...
	plan_ft_c2c_y= FFTW_PLAN_MANY_DFT(1, Ny_plan, (Nz/2+1)*local_Ny,
		reinterpret_cast<FFTW_COMPLEX*>(X_3d.data()), NULL,
		(Nz/2+1)*local_Ny, 1,
		reinterpret_cast<FFTW_COMPLEX*>(X_3d.data()), NULL,
		(Nz/2+1)*local_Ny, 1,
	...
	plan_ift_c2c_y = FFTW_PLAN_MANY_DFT(1, Ny_plan, (Nz/2+1)*local_Ny,
		reinterpret_cast<FFTW_COMPLEX*>(X_3d.data()), NULL,
		(Nz/2+1)*local_Ny, 1,
		reinterpret_cast<FFTW_COMPLEX*>(X_3d.data()), NULL,
		(Nz/2+1)*local_Ny, 1,
	...
	plan_ft_r2c_z = FFTW_PLAN_MANY_DFT_R2C(2, Nyz_plan, local_Nx,
	...
	plan_ift_c2r_z = FFTW_PLAN_MANY_DFT_C2R(2, Nyz_plan, local_Nx,
		reinterpret_cast<FFTW_COMPLEX*>(X_3d.data()), NULL,
		1, (Nz/2+1)*Ny,
		reinterpret_cast<DP*>(X_3d.data()), NULL,
		1, (Nz+2)*Ny,

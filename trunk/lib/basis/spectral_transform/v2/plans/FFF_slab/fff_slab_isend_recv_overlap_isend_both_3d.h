...
class FFF_slab_Isend_Recv_overlap_Isend_both_3D : public SpectralPlan {
private:

	FFTW_PLAN plan_ft_c2c_x;
	FFTW_PLAN plan_ift_c2c_x;
	FFTW_PLAN plan_ft_c2r_yz;
	...
	void IFT_c2c_x_and_Isend(Array<complx,3> A);
	void IFT_c2r_yz(Array<DP,3> Ar);

public:

	FFF_slab_Isend_Recv_overlap_Isend_both_3D(int my_id, int numprocs, int N0, int N1, int N2);

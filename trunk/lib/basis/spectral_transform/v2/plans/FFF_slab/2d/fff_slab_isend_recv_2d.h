...
class FFF_slab_Isend_Recv_2D : public SpectralPlan_Slab_2D {
private:
	...
	void FT_r2c_z(Array<DP,2> Ar);
	void FT_c2c_x(Array<complx,2> A);
	void IFT_c2c_x(Array<complx,2> A);
	void IFT_c2r_z(Array<DP,2> Ar);

public:

	FFF_slab_Isend_Recv_2D(int my_id, int numprocs, int num_iter, size_t Nx, size_t Nz);

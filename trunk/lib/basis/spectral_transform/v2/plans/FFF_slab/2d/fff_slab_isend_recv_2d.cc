...
#include "fff_slab_isend_recv_2d.h"


	
//*********************************************************************************************
FFF_slab_Isend_Recv_2D::FFF_slab_Isend_Recv_2D(int my_id, int numprocs, int num_iter, size_t Nx, size_t Nz): SpectralPlan_Slab_2D(my_id, numprocs, Nx, Nz)
{
...
DP FFF_slab_Isend_Recv_2D::f(int rx, int rz)
{
	DP k0 = 1;
	DP x,z;
	DP L=2*M_PI;
...
void FFF_slab_Isend_Recv_2D::Init_array()
{
...
void FFF_slab_Isend_Recv_2D::Normalize(Array<complx,2> A)
{
	A /= (DP(Nx) * DP(Nz));
}

//*************************************************************************************
void FFF_slab_Isend_Recv_2D::Forward_transform(Array<DP,2> Ar, Array<complx,2> A)
{
...
void FFF_slab_Isend_Recv_2D::Inverse_transform(Array<complx,2> A, Array<DP,2> Ar)
{
	cout << "FFF_slab_Isend_Recv_2D " << A.shape() << endl;
	cout << "FFF_slab_Isend_Recv_2D " << Ar.shape() << endl;
...
void FFF_slab_Isend_Recv_2D::Transpose(Array<DP,2> Ar, Array<complx,2> A){
	Isend_z(Ar);
	Recv_x(A);
}

void FFF_slab_Isend_Recv_2D::Transpose(Array<complx,2> A, Array<DP,2> Ar){
	Isend_x(A);
	Recv_z(Ar);
}

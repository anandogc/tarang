...
#include "ssf_slab_isend_recv_3d.h"


	
//*********************************************************************************************
SSF_slab_Isend_Recv_3D::SSF_slab_Isend_Recv_3D(int my_id, int numprocs, int num_iter, int Nx, int Ny, int Nz): SpectralPlan_Slab_3D(my_id, numprocs, Nx, Ny, Nz)
{
...
void SSF_slab_Isend_Recv_3D::Init_array()
{
	DP k0 = 1;
	DP x,y,z;
	...
	for (int rx=0; rx<Nx; rx++)
		for (int ry=0; ry<local_Ny; ry++)
			for (int rz=0; rz<Nz; rz++)
			{
				...
				z = rz*Lz/Nz;
				Xr_3d(rx, ry, rz) = 4*sin(k0*x)*cos(k0*y)*cos(k0*z);
			}
}

//*************************************************************************************

void SSF_slab_Isend_Recv_3D::Normalize(Array<complx,3> A)
{
...
void SSF_slab_Isend_Recv_3D::Forward_transform(string sincostr_option, Array<DP,3> Ar, Array<complx,3> A)
{
...
void SSF_slab_Isend_Recv_3D::Inverse_transform(string sincostr_option, Array<complx,3> A, Array<DP,3> Ar)
{
	IFT_c2r_z(A);
...
//Transpose
void SSF_slab_Isend_Recv_3D::Transpose(Array<DP,3> Ar, Array<complx,3> A) {
	Alltoall(Ar,A,XZ);
}

void SSF_slab_Isend_Recv_3D::Transpose(Array<complx,3> A, Array<DP,3> Ar) {
	Alltoall(A,Ar,YZ);
}


void SSF_slab_Isend_Recv_3D::SinCostr_x(char sincostr_option, Array<DP,3> Ar)
{
...
void SSF_slab_Isend_Recv_3D::ISinCostr_x(char sincostr_option, Array<DP,3> Ar)
{
    if (sincostr_option == 'S') {
		ArrayShiftLeft(Ar, 'X');
...
void SSF_slab_Isend_Recv_3D::SinCostr_y(char sincostr_option, Array<complx,3> Ar)
{
	if (sincostr_option == 'S') {
...
void SSF_slab_Isend_Recv_3D::ISinCostr_y(char sincostr_option, Array<complx,3> Ar)
{
    if (sincostr_option == 'S') {
		ArrayShiftLeft(Ar, 'Y');
		for (int lx=0; lx<local_Nx; lx++)
...
void SSF_slab_Isend_Recv_3D::FT_r2c_z(Array<complx,3> A)
{
	FFTW_EXECUTE_DFT_R2C(plan_ft_r2c_z, reinterpret_cast<DP*>(A.data()), reinterpret_cast<FFTW_COMPLEX*>(A.data()));
}


void SSF_slab_Isend_Recv_3D::IFT_c2r_z(Array<complx,3> A)
{

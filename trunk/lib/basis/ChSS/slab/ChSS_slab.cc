...
ChFF_SLAB::ChFF_SLAB()
{
	...
	if (Ny > 1) {
		global.field.shape_complex_array = spectralTransform.local_Ny, global.field.Nz/2, global.field.Nx;
        global.field.shape_real_array = spectralTransform.local_Ny, global.field.Nz, global.field.Nx;
		
		BasicIO::shape_full_complex_array = Ny,Nz/2,Nx;
		BasicIO::shape_full_real_array = Ny,Nz,Nx;
		...
		if (global.io.N_in_reduced.size()==3)
			BasicIO::shape_in_reduced_array = global.io.N_in_reduced[1],global.io.N_in_reduced[2]/2,global.io.N_in_reduced[0];
		
		if (global.io.N_out_reduced.size()==3)
			BasicIO::shape_out_reduced_array = global.io.N_out_reduced[1],global.io.N_out_reduced[2]/2,global.io.N_out_reduced[0];
		
		BasicIO::Fourier_directions = 0,0,0;
	}
		...
		BasicIO::shape_full_complex_array = 1,Nz/2,Nx;
		BasicIO::shape_full_real_array = 1,Nz,Nx;
		...
		if (global.io.N_in_reduced.size()==3)
			BasicIO::shape_in_reduced_array = 1,global.io.N_in_reduced[2]/2,global.io.N_in_reduced[0];
		
		if (global.io.N_out_reduced.size()==3)
			BasicIO::shape_out_reduced_array = 1,global.io.N_out_reduced[2]/2,global.io.N_out_reduced[0];

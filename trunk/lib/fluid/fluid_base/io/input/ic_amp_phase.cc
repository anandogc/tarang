...
void FluidIO::Put_vector_amp_phase_comp_conj(FluidVF U, int lx, int ly, int lz,  Real ek, Real hk_by_k, Real phase_plus, Real phase_minus)
{
  ...
  if (kkmag > MYEPS) {
    if ( kkperp > MYEPS) {
      vpll = -u2 * sin(theta);
      vh1 =  u2 * cos(theta)*cos(phi) + u1 * sin(phi);
      vh2 =  u2 * cos(theta)*sin(phi) - u1 * cos(phi);
    }
